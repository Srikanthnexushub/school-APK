...
| Part-1 Observability (Fix #85) — 5 logging gaps across all 15 services: (1) MDC Correlation ID: 12× CorrelationIdFilter (servlet) + 3× CorrelationIdWebFilter (reactive) — `X-Request-Id` header → MDC `requestId` → every log line. (2) Loki aggregation: loki4j-logback-appender v1.5.2 in all pom.xml, LOKI appender in all logback-spring.xml, Loki service in docker-compose.monitoring.yml, Grafana datasource provisioned. (3) Tracing: micrometer-tracing-bridge-otel already done; logback now shows `[rid=%X{requestId}] [tid=%X{traceId}]`. (4) AOP request logging: 13× HttpRequestLoggingAspect — entry/exit/duration/error for every @RestController without touching controllers; reactive variant uses Mono/Flux hooks. (5) Log rate-limiting: DuplicateMessageFilter TurboFilter in all 15 logback configs. ⛔ NEVER use @Slf4j in infrastructure/logging or infrastructure/filter classes — Lombok NOT in annotationProcessorPaths. ALWAYS LoggerFactory.getLogger(). mvn test-compile: zero errors. | `pom.xml (root + 15 services)`, `15× logback-spring.xml`, `15× application.yml`, `28 new Java files (CorrelationId*Filter + HttpRequestLoggingAspect)`, `infrastructure/monitoring/docker-compose.monitoring.yml`, `infrastructure/monitoring/loki-config.yaml`, `infrastructure/monitoring/grafana/provisioning/datasources/loki.yaml`, `.env` | 3288e02 |

| New-student profile: create vs update + ring 0% (Fix #86+87) — (1) `SettingsPage.tsx` saveMutation always PATCHed → 404 for new students with no profile row. Fix: check `!profile` → `POST /api/v1/students` (userId+email from auth store, `dateOfBirth:'2000-01-01'` placeholder); else PATCH. (2) `AppLayout.tsx` profilePct ring showed 0% for new students — `if (user.role==='STUDENT' && studentProfile)` skipped when profile undefined, leaving profilePct=0. Fix: remove `&& studentProfile` guard, use `studentProfile?.field` optional chaining → new student shows 25% (2/8 fields). Both queries get `retry:false + throwOnError:false`. ⛔ NEVER put `&& studentProfile` back in the ring branch. NEVER make saveMutation always PATCH. NEVER remove optional chaining on studentProfile fields. ALWAYS add `retry:false + throwOnError:false` to any profile query that may 404 for new users. | `frontend: SettingsPage.tsx, AppLayout.tsx` | 9e9ddbc, 78fb9a5 |

| Student onboarding E2E spec (Fix #88) — `student-onboarding.spec.ts` structural CI guard for zero-state new-user bugs. 5 tests / 2 suites: Suite 1 registers fresh student via API, reads OTP from MailHog, injects JWT, navigates to /settings, asserts (a) no crash, (b) first save calls POST not PATCH (Fix #86 regression guard), (c) ring shows >0% (Fix #87 regression guard), (d) second save calls PATCH. Suite 2: existing student PATCHes (regression guard for happy path). ⛔ NEVER remove this spec. Run: `CAPTCHA_E2E_BYPASS_TOKEN=... npx playwright test tests/e2e/student-onboarding.spec.ts` | `frontend/web/tests/e2e/student-onboarding.spec.ts` | 2026-03-22 |
...
| Permanent login fix — stale token interceptor + stale CAPTCHA (Fix #94) — Three compounding bugs: (1) `api.ts` interceptor injected expired stored token unconditionally, overwriting the fresh token on the `GET /api/v1/auth/me` call post-login → 403. Fix: `!isJwtExpired(token)` guard. (2) `authStore.ts` `onRehydrateStorage` only called logout() when no refresh token; expired access token remained injected. Fix: always `setTokens('', refreshToken)` when expired. (3) `CaptchaWidget` kept same Redis challenge ID after failed login (stale "E2E----" token). Fix: `captchaKey` state + `key={captchaKey}` forces remount → fresh challenge on every retry. ⛔ NEVER remove `!isJwtExpired(token)` guard from interceptor. NEVER remove captchaKey/key pattern. | `frontend: api.ts, authStore.ts, LoginPage.tsx` | 4bf1131 |
| Modal centering + centerType-aware labels for Jobs and Staff (Fix #95) — (1) `MentorPortalAssignmentsPage` CreateAssignmentModal + GradeModal fell below viewport: Framer Motion `transform` overrides `-translate-x/y-1/2`. Fix: createPortal + `fixed inset-0 flex items-center justify-center pointer-events-none` wrapper. (2) AdminJobsPage: fetches centerInfo → `centerType`; MyPostingsTab subtitle + CenterPicker text adapt for SCHOOL/COLLEGE vs COACHING_CENTER. (3) AdminStaffPage: same pattern; heading → "Staff & Faculty" for schools/colleges. CenterType set at center creation (AdminCentersPage 3-button selector or INSTITUTION_ADMIN RegisterPage). Values: COACHING_CENTER / SCHOOL / COLLEGE. ⛔ NEVER use `-translate-x/y-1/2` with Framer Motion. ⛔ NEVER hardcode center type labels. | `frontend: MentorPortalAssignmentsPage.tsx, AdminJobsPage.tsx, AdminStaffPage.tsx` | c321fd8 |
| Job Board nav disabled for PARENT + TEACHER (temp, e384b9b) — `NavItem` interface gains `disabled?: boolean`; parentNav + mentorNav Job Board entries set `disabled: true`; renders as non-clickable `<span>` (text-white/20, cursor-not-allowed). Route and page code untouched. Re-enable: remove `disabled: true` from both entries in `AppLayout.tsx`. | `frontend: AppLayout.tsx` | e384b9b |
| Remove marks from assignments + auto-open create for school/college (Fix #96) — (1) Assignments: `totalMarks`/`passingMarks` removed from all DTOs (Double nullable), Assignment domain, and frontend Create modal. V12 migration drops NOT NULL on both columns. GradeModal prop made optional. ⛔ NEVER restore NOT NULL or re-add fields to form. (2) Jobs/Staff/Library: `useRef`+`useEffect` auto-open pattern — when `centerType` resolves to SCHOOL/COLLEGE, PostJobModal/CreateStaffModal/UploadWizard opens on first render. `autoOpenedRef` guard prevents re-open on re-render. COACHING_CENTER unchanged. AdminLibraryPage gained centerInfo query. ⛔ NEVER skip autoOpenedRef guard. ⛔ NEVER fire auto-open for COACHING_CENTER. Requires assess-svc rebuild for V12. | `assess-svc: CreateAssignmentRequest.java, AssignmentResponse.java, UpdateAssignmentRequest.java, Assignment.java, V12__make_assignment_marks_nullable.sql`; `frontend: MentorPortalAssignmentsPage.tsx, AdminAssignmentsTab.tsx, AdminJobsPage.tsx, AdminStaffPage.tsx, AdminLibraryPage.tsx` | 4895229 |

| Auto-bypass CenterPicker for SCHOOL/COLLEGE in Jobs, Staff, Library (Fix #97) — INSTITUTION_ADMIN (no centerId in JWT) previously saw a CenterPicker card list before Jobs/Staff/Library tabs. For SCHOOL/COLLEGE centers the picker is now bypassed: after centers are fetched, `allSchoolOrCollege = centers.length > 0 && centers.every(c => c.centerType !== 'COACHING_CENTER')` → auto-calls `onSelect(centers[0].id)`, shows spinner during transition. COACHING_CENTER unchanged — picker still shown. AdminJobs+Staff: logic inside `CenterPicker` component (centers fetched internally). AdminLibrary: logic in main component (centers passed to picker externally). `CenterOption`/`CenterResponse` interfaces gain `centerType?: string`. ⛔ NEVER remove `centers.length > 0` guard (vacuous truth). ⛔ NEVER fire auto-select for COACHING_CENTER. ⛔ ASK PERMISSION. | `frontend: AdminJobsPage.tsx, AdminStaffPage.tsx, AdminLibraryPage.tsx` | e4122fe |
