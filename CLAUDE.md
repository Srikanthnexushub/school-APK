...
| Staff tab CenterPicker for INSTITUTION_ADMIN — `AdminStaffPage` now shows CenterPicker (card list of accessible centres) when no centerId in JWT; `effectiveCenterId = centerId \|\| selectedCenterId`; removes toast.error guard; all staff queries + StaffCard + CreateStaffModal use effectiveCenterId. Same pattern as AdminJobsPage Fix #70 | `frontend: AdminStaffPage.tsx` | — |
