...
public class EnrollmentService implements EnrollStudentUseCase {
    ...
    @Transactional(readOnly = true)
    public Page<EnrollmentResponse> listEnrollments(UUID examId, AuthPrincipal principal, Pageable pageable) {
        Exam exam = examRepository.findById(examId)
                .orElseThrow(() -> new ExamNotFoundException(examId));
        if (!principal.belongsToCenter(exam.getCenterId())) {
            throw new AssessAccessDeniedException();
        }
        List<EnrollmentResponse> all = enrollmentRepository.findByExamId(examId).stream()
