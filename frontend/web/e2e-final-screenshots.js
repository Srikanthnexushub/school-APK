...
async function loginAndInject(page, email, password) {
  const resp = await page.evaluate(async ({email, password}) => {
    const r = await fetch('http://localhost:8180/api/v1/auth/login', {
      method: 'POST',
  ...
  console.log(`Login response keys for ${email}:`, Object.keys(resp));
  if (!resp.accessToken) {
  ...
  console.log(`JWT payload for ${email}:`, JSON.stringify(jwtPayload));
...
(async () => {
  ...
  // Try to find a "Graded" tab/button
  const gradedBtn = page.locator('text=Graded').first();
  if (await gradedBtn.count() > 0) {
    await gradedBtn.click();
    console.log('Clicked Graded button');
  } else {
    console.log('No Graded button found, trying tab...');
