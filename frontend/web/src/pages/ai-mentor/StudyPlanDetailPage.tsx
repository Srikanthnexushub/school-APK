...
import { CheckCircle2, ArrowLeft, BookOpen, Lightbulb } from 'lucide-react';
...
function AiSuggestionsPanel({ weakSubjects }: { weakSubjects: string[] }) {
  const tips = [
    'Focus on your weakest topics first each morning when focus is highest.',
    'Use the Pomodoro technique: 25 min study, 5 min break.',
    'Review yesterday\'s material for 10 minutes before starting new content.',
    'Attempt at least one mock test per week to benchmark progress.',
    'Submit doubts immediately — unresolved questions compound over time.',
    weakSubjects.length > 0
      ? `Prioritise ${weakSubjects[0]} — it\'s your most critical gap.`
      : 'Maintain consistency: even 30 minutes daily beats irregular marathons.',
  ];
      ...
      {weakSubjects.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {weakSubjects.map(s => (
              <span key={s} className="px-2 py-0.5 rounded-full text-xs bg-red-500/15 text-red-400 border border-red-500/20">
                {s}
              </span>
            ))}
          </div>
  ...
  return (
    <div className="glass rounded-2xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-white/5">
        ...
        <span className="text-white/40 text-xs">{done}/{items.length} done</span>
      </div>
            ...
            >
              <CheckCircle2
                className={cn(
                  ...
                  item.completed ? 'text-emerald-400' : 'text-white/20 hover:text-emerald-400'
                )}
              />
            </button>

            <div className="flex-1 min-w-0">
              <p className={cn('text-sm text-white/80', item.completed && 'line-through')}>
                {item.topic}
              </p>
              {item.estimatedMinutes && (
                <p className="text-xs text-white/30 mt-0.5">{item.estimatedMinutes} min estimated</p>
              )}
            </div>
    ...
    return (
      <div className="p-6 max-w-7xl mx-auto">
        <div className="glass rounded-2xl p-12 text-center space-y-4">
          ...
          <button onClick={() => planQuery.refetch()} className="btn-primary text-sm">
            Retry
          </button>
        </div>
      </div>
    ...
    >
      ...
      <div className="glass rounded-2xl p-6 space-y-4">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-xl font-bold text-white">{plan.targetExam ?? 'Study Plan'}</h1>
          ...
          <Badge variant={plan.status === 'ACTIVE' ? 'info' : plan.status === 'COMPLETED' ? 'success' : 'default'}>
            {plan.status}
          </Badge>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="text-white font-semibold">{pct}% — {done}/{total} items</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {Object.keys(grouped).length === 0 ? (
            ...
            Object.entries(grouped).map(([subject, subjectItems]) => (
              <SubjectGroup
                ...
                subject={subject}
                items={subjectItems}
                onToggle={(itemId, completed) => toggleMutation.mutate({ itemId, completed })}
        ...
        {/* Side panel */}
        <div>
          <AiSuggestionsPanel weakSubjects={plan.weakSubjects ?? []} />
        </div>
      </div>
    </motion.div>
  );
