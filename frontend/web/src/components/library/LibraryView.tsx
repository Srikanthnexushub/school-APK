// Shared read-only library view — used by student, parent, and any role
// that has a resolved centerId.
...
export function LibraryView({ centerId, isResolvingCenter = false, emptyHint }: LibraryViewProps) {
  const [filterType, setFilterType] = useState<string>('ALL');
  const [search, setSearch] = useState('');

  const { data: content = [], isLoading: contentLoading } = useQuery<ContentItem[]>({
    queryKey: ['library-content', centerId],
    queryFn: async () => {
  ...
  const isLoading = isResolvingCenter || contentLoading;
  ...
  if (!centerId) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-white/25 text-sm mt-1">{emptyHint ?? 'Complete an assessment to unlock your study library.'}</p>
      </div>
    );
  }
